...
description: Solutions for common issues when setting up and running Oppvera Watch, including Docker Desktop and first-time install problems.
---

## Docker Desktop is not running

**Symptom:** Errors mentioning Docker, "Cannot connect to the Docker daemon", or database connection failures when you run `pnpm local`.

**Cause:** Docker Desktop is installed but not open, or still starting up.

**Fix:**

1. Open **Docker Desktop** from Applications (Mac) or the Start menu (Windows)
2. Wait until it shows the engine is **running** (whale icon in the menu bar on Mac)
3. Run `docker ps` in Terminal or PowerShell—you should see a table header, not an error
4. Run `pnpm local` again

Keep Docker Desktop open the whole time you use Oppvera Watch.

---

## Docker Desktop is not installed

**Symptom:** `docker: command not found` or Windows cannot find Docker.

**Fix:** Follow [Local Setup — Install Docker Desktop](/local-setup#install-docker-desktop), then restart your terminal and try again.
...
**Cause:** The Redis Docker container is not running—usually because Docker Desktop stopped.
...
If the containers aren't listed, open Docker Desktop first, then restart with `pnpm local`. If Docker itself isn't running, start it first.
...
**Fix:** Open the provider connection again, complete the full sign-in flow (including any 2FA or confirmation steps), and wait for the checkmark to appear in the Oppvera Watch UI before closing the browser window.
...
Note: these control sign-in to the Oppvera Watch web app itself. They are separate from the Google / Gemini / AI Overview provider connections used for monitoring.
...
**Symptom:** `node scripts/run-compose.mjs bootstrap` fails during Docker build or startup.

**Cause:** Missing Docker resources, insufficient disk/RAM, or a build error in the cloned repository.

**Fix:** Check Docker is running and retry:

```bash
node scripts/run-compose.mjs bootstrap
```

You can also run `pnpm self-host:build` directly. Bootstrap always builds web, agent, and Postgres from the cloned repository rather than pulling upstream OneGlanse images.
