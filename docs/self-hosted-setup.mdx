...
description: Advanced VPS deployment for Oppvera Watch. Not recommended for classroom or marketing learners—use local setup instead.
---

## Not for classroom use

**Marketing and GEO learners should use [Local Setup](/local-setup) on a laptop**, not this guide.

Self-hosted VPS setup is for advanced experimentation only. It adds complexity you do not need for learning GEO, and it raises separate compliance questions:

- **AI provider terms.** Automated browser access to ChatGPT, Gemini, and similar products may be restricted by each provider's terms of use. Local learning on your own account is simpler to reason about than unattended server automation.
- **Residential proxy services.** VPS deployments typically require a paid third-party proxy (for example ThorData). That is a separate subscription with its own vendor terms—not included in Oppvera Watch's MIT license.
- **Server operations.** You are responsible for domain, TLS, firewall, and VPS billing.

If you are teaching a course, point students at **local setup** and skip VPS unless you have explicitly cleared the operational and licensing context with your organization.
...
bash <(curl -fsSL https://raw.githubusercontent.com/oppvera/oppvera-watch/main/scripts/setup-vps.sh)
```
...
When the script prompts for `Your domain for the app`, enter the hostname you want to serve Oppvera Watch on, such as `app.yourdomain.com`. The script configures nginx and requests the TLS certificate for that exact domain.

Or clone first and run locally:

```bash
git clone https://github.com/oppvera/oppvera-watch /tmp/oppvera-watch
bash /tmp/oppvera-watch/scripts/setup-vps.sh
```
...
The VPS flow builds the app from the cloned repository:

- it runs `docker compose up --build` for the web, agent, and Postgres services
- it does not pull upstream OneGlanse published images by default
- `git pull` updates the repo; rerun bootstrap to rebuild and restart from your fork's source

---

## Manual setup

This guide deploys Oppvera Watch on a fresh Ubuntu 22.04 / 24.04 VPS. At the end you'll have:

- Oppvera Watch running behind nginx with HTTPS
...
- Oppvera Watch collects responses from real provider UIs, not official model APIs
...
```bash
git clone https://github.com/oppvera/oppvera-watch /home/$USER/oppvera-watch
cd /home/$USER/oppvera-watch
cp .env.example .env
```
...
- Oppvera Watch collects from authenticated web UIs, so network reputation matters
...
On a fresh server, this can take several minutes because Docker builds the app images from source and pulls infrastructure images (Redis, ClickHouse).

This builds from the cloned repository and starts:
...
```bash
sudo nano /etc/nginx/sites-available/oppvera-watch
```
...
```bash
sudo ln -s /etc/nginx/sites-available/oppvera-watch /etc/nginx/sites-enabled/
...
Before you run prompts from the VPS, make sure `THORDATA_PROXY_API_URL` in `/home/$USER/oppvera-watch/.env` points to your real residential proxy endpoint. If you deployed with a placeholder or later changed the proxy configuration, update that value and rerun:
